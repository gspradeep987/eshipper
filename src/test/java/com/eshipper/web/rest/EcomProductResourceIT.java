...
class EcomProductResourceIT {
  ...
  private static final String ENTITY_API_URL = "/api/ecom-products";
  ...
  @Test
  @Transactional
  void createEcomProduct() throws Exception {
    ...
    restEcomProductMockMvc
      .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(ecomProductDTO)))
    ...
    assertThat(testEcomProduct.getIsInsured()).isEqualTo(DEFAULT_IS_INSURED);
  }

  @Test
  @Transactional
  void createEcomProductWithExistingId() throws Exception {
  ...
  @Test
  @Transactional
  void getAllEcomProducts() throws Exception {
  ...
  void getAllEcomProductsWithEagerRelationshipsIsEnabled() throws Exception {
    when(ecomProductServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

    restEcomProductMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

    verify(ecomProductServiceMock, times(1)).findAllWithEagerRelationships(any());
  }

  @SuppressWarnings({ "unchecked" })
  void getAllEcomProductsWithEagerRelationshipsIsNotEnabled() throws Exception {
    when(ecomProductServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
  ...
  @Test
  @Transactional
  void getEcomProduct() throws Exception {
    // Initialize the database
    ecomProductRepository.saveAndFlush(ecomProduct);

    // Get the ecomProduct
    restEcomProductMockMvc
      .perform(get(ENTITY_API_URL_ID, ecomProduct.getId()))
      .andExpect(status().isOk())
      .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
  ...
  @Test
  @Transactional
  void getNonExistingEcomProduct() throws Exception {
    // Get the ecomProduct
    restEcomProductMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
  }

  @Test
  @Transactional
  void putNewEcomProduct() throws Exception {
    // Initialize the database
    ecomProductRepository.saveAndFlush(ecomProduct);
    ...
    restEcomProductMockMvc
      .perform(
        put(ENTITY_API_URL_ID, ecomProductDTO.getId())
    ...
    assertThat(testEcomProduct.getIsInsured()).isEqualTo(UPDATED_IS_INSURED);
  }

  @Test
  @Transactional
  void putNonExistingEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
  ...
  @Test
  @Transactional
  void putWithIdMismatchEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
  ...
  @Test
  @Transactional
  void putWithMissingIdPathParamEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
    ...
    // If url ID doesn't match entity ID, it will throw BadRequestAlertException
    restEcomProductMockMvc
      .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(ecomProductDTO)))
  ...
  @Test
  @Transactional
  void partialUpdateEcomProductWithPatch() throws Exception {
    // Initialize the database
    ecomProductRepository.saveAndFlush(ecomProduct);

    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();

    // Update the ecomProduct using partial update
    EcomProduct partialUpdatedEcomProduct = new EcomProduct();
    partialUpdatedEcomProduct.setId(ecomProduct.getId());

    partialUpdatedEcomProduct
      .width(UPDATED_WIDTH)
      .height(UPDATED_HEIGHT)
      .weight(UPDATED_WEIGHT)
      .goodsValue(UPDATED_GOODS_VALUE)
    ...
    restEcomProductMockMvc
      .perform(
        patch(ENTITY_API_URL_ID, partialUpdatedEcomProduct.getId())
          .contentType("application/merge-patch+json")
          .content(TestUtil.convertObjectToJsonBytes(partialUpdatedEcomProduct))
      )
      .andExpect(status().isOk());
  ...
  @Test
  @Transactional
  void fullUpdateEcomProductWithPatch() throws Exception {
    // Initialize the database
    ecomProductRepository.saveAndFlush(ecomProduct);
  ...
  @Test
  @Transactional
  void patchNonExistingEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
    ...
    restEcomProductMockMvc
      .perform(
        patch(ENTITY_API_URL_ID, ecomProductDTO.getId())
          .contentType("application/merge-patch+json")
          .content(TestUtil.convertObjectToJsonBytes(ecomProductDTO))
      )
      .andExpect(status().isBadRequest());
  ...
  @Test
  @Transactional
  void patchWithIdMismatchEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
  ...
  @Test
  @Transactional
  void patchWithMissingIdPathParamEcomProduct() throws Exception {
    int databaseSizeBeforeUpdate = ecomProductRepository.findAll().size();
    ecomProduct.setId(count.incrementAndGet());
    ...
    // If url ID doesn't match entity ID, it will throw BadRequestAlertException
    restEcomProductMockMvc
      .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(ecomProductDTO)))
      .andExpect(status().isMethodNotAllowed());
  ...
  @Test
  @Transactional
  void deleteEcomProduct() throws Exception {
    // Initialize the database
    ecomProductRepository.saveAndFlush(ecomProduct);
    ...
    restEcomProductMockMvc
      .perform(delete(ENTITY_API_URL_ID, ecomProduct.getId()).accept(MediaType.APPLICATION_JSON))
