...
public class AffiliateResource {
    ...
    public ResponseEntity<List<AffiliateDTO>> getAllAffiliates(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("elasticsearchaffiliate-is-null".equals(filter)) {
            log.debug("REST request to get all Affiliates where elasticSearchAffiliate is null");
            return new ResponseEntity<>(affiliateService.findAllWhereElasticSearchAffiliateIsNull(),
                    HttpStatus.OK);
        }
