...
public class EcomMarkupPrimaryServiceImpl implements EcomMarkupPrimaryService {
  ...
  public EcomMarkupPrimaryServiceImpl(
    EcomMarkupPrimaryRepository ecomMarkupPrimaryRepository,
    EcomMarkupPrimaryMapper ecomMarkupPrimaryMapper
  ) {
  ...
  @Override
  public Optional<EcomMarkupPrimaryDTO> partialUpdate(EcomMarkupPrimaryDTO ecomMarkupPrimaryDTO) {
    log.debug("Request to partially update EcomMarkupPrimary : {}", ecomMarkupPrimaryDTO);

    return ecomMarkupPrimaryRepository
      .findById(ecomMarkupPrimaryDTO.getId())
      .map(
        existingEcomMarkupPrimary -> {
          ecomMarkupPrimaryMapper.partialUpdate(existingEcomMarkupPrimary, ecomMarkupPrimaryDTO);
          return existingEcomMarkupPrimary;
        }
      )
      .map(ecomMarkupPrimaryRepository::save)
  ...
  public Optional<EcomMarkupPrimaryDTO> findOne(Long id) {
    ...
    return ecomMarkupPrimaryRepository.findById(id).map(ecomMarkupPrimaryMapper::toDto);
  }
