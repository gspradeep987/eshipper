...
export class SisyphusSubJobComponent implements OnInit {
  ...
  protected onSuccess(data: ISisyphusSubJob[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sisyphus-sub-job'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
