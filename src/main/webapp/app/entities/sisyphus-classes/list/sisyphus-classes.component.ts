...
export class SisyphusClassesComponent implements OnInit {
  ...
  protected onSuccess(data: ISisyphusClasses[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/sisyphus-classes'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
