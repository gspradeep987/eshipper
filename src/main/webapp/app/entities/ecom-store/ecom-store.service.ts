...
export class EcomStoreService {
  ...
  create(ecomStore: IEcomStore): Observable<EntityResponseType> {
    return this.http.post<IEcomStore>(this.resourceUrl, ecomStore, { observe: 'response' });
  }

  update(ecomStore: IEcomStore): Observable<EntityResponseType> {
    return this.http.put<IEcomStore>(this.resourceUrl, ecomStore, { observe: 'response' });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IEcomStore>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }

  query(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<IEcomStore[]>(this.resourceUrl, { params: options, observe: 'response' });
  }
