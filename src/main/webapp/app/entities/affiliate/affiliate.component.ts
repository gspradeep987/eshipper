...
export class AffiliateComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.affiliateService
      ...
      .subscribe(
        (res: HttpResponse<IAffiliate[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IAffiliate): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInAffiliates(): void {
    this.eventSubscriber = this.eventManager.subscribe('affiliateListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAffiliate[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
